...
test("Date examples", function() {
    ...
    var report = env.validate({ "birthday" : "1.1"}, schema);
    equal(report.errors.length, 1, "Invalid date");

    report = env.validate({ "birthday" : "1992-10-27"}, schema);
    equal(report.errors.length, 0, "Valid date");
});


test("Datetime examples", function() {
    var schema = env.createSchema({
	"properties": {
	    "created": {
	        "type": "string",
    ...
    var report = env.validate({ "created" : "11134123-143"}, schema);
    equal(report.errors.length, 1, "Invalid date-time");

    report = env.validate({ "created" : "2015-02-12T12:25:39.398952Z"}, schema);
    equal(report.errors.length, 0, "Valid date-time (python format)");

    report = env.validate({ "created" : "2015-02-12T12:25:39.100Z"}, schema);
    equal(report.errors.length, 0, "Valid date-time (javascript format)");
});

test("Email examples", function() {
    var schema = env.createSchema({
	"properties": {
	    "email": {
	        "type": "string",
    ...
    var report = env.validate({ "email" : "email-jujuca"}, schema);
    equal(report.errors.length, 1, "Invalid e-mail");

    report = env.validate({ "email": "junior@globo.com" }, schema);
    equal(report.errors.length, 0, "Valid e-mail");
});

test("URI examples", function() {
    var schema = env.createSchema({
	"properties": {
	    "uri": {
	        "type": "string",
    ...
    var report = env.validate({ "uri" : "ftp://blah-balah.com.br/gs.git"}, schema);
    equal(report.errors.length, 0, "Valid URI");

    report = env.validate({ "uri": "+41 43 000 00 00 Grüezi" }, schema);
    equal(report.errors.length, 1, "Invalid URI");
});

test("URL examples", function() {
    var schema = env.createSchema({
	"properties": {
	    "uri": {
	        "type": "string",
    ...
    report = env.validate({ "uri": "blah--juu" }, schema);
    equal(report.errors.length, 1, "Invalid URI");
});
